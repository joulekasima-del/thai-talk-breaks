# Week 2 — Daily Life (Days 8–14)

All lessons follow the locked six-step structure (LDTKB-006: picture → Karaoke → English → script → audio → activity), the 5-tone diacritic system (LDTKB-025), and gender branching where the phrase includes a polite ending (LDTKB-024). Grammar rule: คะ for questions, ค่ะ for statements (confirmed by Joule, 19 August 2026). Draft only — none of this has native-speaker review, audio, or images yet, unlike the Week 1 pilot content.

---

## Day 8 — Directions
**Type:** word set (no gender branch, like Day 2's numbers)

| Word | Karaoke | Tone pattern | Meaning |
|---|---|---|---|
| ตรงไป | dtrong bpai | mid – mid | Straight ahead |
| เลี้ยวซ้าย | líeow sái | high – high | Turn left |
| เลี้ยวขวา | líeow khwǎa | high – rising | Turn right |
| จอดตรงนี้ | jòrt dtrong-níi | low – mid – high | Stop here |

## Day 9 — Telling time
**Phrase:** What time is it now? (question — คะ)

| Voice | Karaoke | Tone pattern |
|---|---|---|
| Male | gìi-mohng-láew kráp | rising – mid – high – high |
| Female | gìi-mohng-láew ká | rising – mid – high – high |

Script: กี่โมงแล้วครับ / กี่โมงแล้วคะ

**Common extension:** Thai speakers commonly open this question with "excuse me" first — a polite two-part phrase:

| Voice | Karaoke | Script |
|---|---|---|
| Male | khǎaw-thôht ná kráp, gìi-mohng-láew kráp | ขอโทษนะครับ กี่โมงแล้วครับ |
| Female | khǎaw-thôht ná-ká, gìi-mohng-láew ká | ขอโทษนะคะ กี่โมงแล้วคะ |

(Note: ขอโทษ reuses the same word taught in Day 20 as "sorry" — here it functions as "excuse me," a natural overlap in Thai. Female "นะคะ" spelling per LDTKB-041.)

## Day 10 — Days of the week / today, tomorrow
**Type:** word set (no gender branch)

| Word | Karaoke | Tone pattern | Meaning |
|---|---|---|---|
| วันนี้ | wan-níi | mid – high | Today |
| พรุ่งนี้ | phrûng-níi | falling – high | Tomorrow |
| เมื่อวานนี้ | mêua-waan-níi | falling – mid – high | Yesterday |

## Day 11 — Weather small talk
**Phrase:** It's really hot today (statement — ค่ะ)

| Voice | Karaoke | Tone pattern |
|---|---|---|
| Male | wan-níi rórn mâak kráp | mid – high – high – falling – high |
| Female | wan-níi rórn mâak kâ | mid – high – high – falling – falling |

Script: วันนี้ร้อนมากครับ / วันนี้ร้อนมากค่ะ

## Day 12 — At the market (more items)
**Phrase:** Do you have anything bigger than this? (question — คะ)

| Voice | Karaoke | Tone pattern |
|---|---|---|
| Male | mii yài gwàa níi mǎi kráp | mid – falling – low – high – rising – high |
| Female | mii yài gwàa níi mǎi ká | mid – falling – low – high – rising – high |

Script: มีใหญ่กว่านี้ไหมครับ / มีใหญ่กว่านี้ไหมคะ

**Also useful — the opposite:**

| Voice | Karaoke | Tone pattern |
|---|---|---|
| Male | mii lék gwàa níi mǎi kráp | mid – high – low – high – rising – high |
| Female | mii lék gwàa níi mǎi ká | mid – high – low – high – rising – high |

Script: มีเล็กกว่านี้ไหมครับ / มีเล็กกว่านี้ไหมคะ ("Do you have anything smaller than this?")

## Day 13 — Ordering drinks/coffee
**Phrase:** I'll take one iced coffee (statement — ค่ะ)

| Voice | Karaoke | Tone pattern |
|---|---|---|
| Male | ao gaa-fae yen nèung gâew kráp | rising – mid – mid – mid – low – falling – high |
| Female | ao gaa-fae yen nèung gâew kâ | rising – mid – mid – mid – low – falling – falling |

Script: เอากาแฟเย็นหนึ่งแก้วครับ / เอากาแฟเย็นหนึ่งแก้วค่ะ

**Sweetness levels — commonly used in Thai cafes (shown as a fill level, lowest to highest sugar):**

| Level | Karaoke | Script |
|---|---|---|
| 🥤▱▱▱▱ | mâi wǎan | ไม่หวาน |
| 🥤▰▱▱▱ | wǎan nói | หวานน้อย |
| 🥤▰▰▰▱ | wǎan bpòk-gà-tì | หวานปกติ |
| 🥤▰▰▰▰ | wǎan mâak | หวานมาก |

*(Fill level shown visually rather than exact percentages, since exact conventions can vary slightly cafe to cafe — the relative order low → high is the reliable, consistent part.)*

**Self-construction note:** unlike earlier lessons, this one should explicitly show learners they can mix and match — swap "กาแฟ" (coffee) for "ชา" (tea), or add a sweetness level from the table above, to build their own real order rather than only ever saying the exact phrase taught.

## Day 14 — Saying goodbye
**Phrase:** See you next time (statement — ค่ะ)

| Voice | Karaoke | Tone pattern |
|---|---|---|
| Male | jer gan mài kráp | mid – mid – mid – high |
| Female | jer gan mài kâ | mid – mid – mid – falling |

Script: เจอกันใหม่ครับ / เจอกันใหม่ค่ะ

---

## Open items (all Week 2 lessons)
1. Native-speaker pronunciation/tone review — none of these have been checked, unlike the Week 1 pilot content.
2. Audio production — not started.
3. Image production — not started.
4. Recognition-tap distractors — can reuse Week 1 pilot phrases as the initial distractor pool per lesson, per the pattern Claude Code already implemented in Checkpoint 3.
